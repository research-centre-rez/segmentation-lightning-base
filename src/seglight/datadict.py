...
def split_train_val(
    cvr_dataset:CVRImgDictLoader,
    val_size = .2,
    seed = 123
):
    rng = np.random.default_rng(seed = seed)
    
    train_dirs,val_dirs = ms.train_test_split(
        cvr_dataset.image_dirs,
        test_size = val_size,
        random_state=seed
    )

    train_ds = CVRImgDictLoader(
        train_dirs,
        cvr_dataset.image_filenames,
        cvr_dataset.patch_shape,
        cvr_dataset.assume_grayscale,
        cvr_dataset.name_mapping,
        cvr_dataset.max_val
    )
    val_ds = CVRImgDictLoader(
        val_dirs,
        cvr_dataset.image_filenames,
        None, #cvr_dataset.patch_shape, THIS must no be here
        cvr_dataset.assume_grayscale,
        cvr_dataset.name_mapping,
        cvr_dataset.max_val
    )
